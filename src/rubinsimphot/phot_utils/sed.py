...
(so, not until needed for the magnitude calculation or resample_sed is called). At that time the min/max/step
...
Method include:
* setSED / set_flat_sed / readSED_flambda / readSED_fnu -- to input information into Sed wavelen/flambda.
...
* resample_sed -- primarily internal use, but may be useful to user. Resamples SED onto specified grid.
* flambda_tofnu / fnuToflambda -- conversion methods, does not affect wavelen gridding.
* redshift_sed -- redshifts the SED, optionally adding dimmingx
* (setupODonnell_ab or setup_ccm_ab) / add_dust -- separated into two components,
...
* calc_adu / calc_mag / calcFlux -- with a Bandpass, calculate the ADU/magnitude/flux of a SED.
* calcFluxNorm / multiply_flux_norm -- handle fluxnorm parameters (from UW LSST database) properly.
...
* setPhiArray -- given a list of bandpasses, sets up the 2-d phiArray (for many_mag_calc) and dlambda value.
* many_mag_calc -- given 2-d phiArray and dlambda, this will return an array of magnitudes (in the same
...
class Sed:
    ...
    def calc_flux(self, bandpass, wavelen=None, fnu=None):
        ...
        # the same sed and same bandpass region - in that case, use self.synchronize_sed() with
    ...
    def multiply_flux_norm(self, flux_norm, wavelen=None, fnu=None):
        ...
        Note that multiply_flux_norm does not regrid self.wavelen/flambda/fnu at all.
        """
    ...
    def renormalize_sed(
        ...
        # Do not use this for calculating specific magnitudes -- use calcfluxNorm and multiply_flux_norm.
    ...
    def setup_phi_array(self, bandpasslist):
        ...
        This is intended to be used once, most likely before using Sed's many_mag_calc many times on many SEDs.
    ...
    def many_flux_calc(self, phiarray, wavelen_step, observed_bandpass_ind=None):
        ...
        .. note: Sed.many_flux_calc `assumes` phiArray has the same wavelenghth
